export const api = {
	...
	content: {
		...
		album: async (albumId: string) => {
			...
			const res = await fetch(`/api/content/album?id=${encodeURIComponent(albumId)}`);
			if (!res.ok) return null;
			return res.json();
		},
		playlist: async (playlistId: string) => {
			if (!playlistId) return null;
			const res = await fetch(`/api/content/playlist?id=${encodeURIComponent(playlistId)}`);
			if (!res.ok) return null;
			return res.json();
		},
		artist: async (artistId: string) => {
			if (!artistId) return null;
			const res = await fetch(`/api/content/artist?id=${encodeURIComponent(artistId)}`);
			if (!res.ok) return null;
			return res.json();
		},
		song: async (songId: string) => {
			if (!songId) return null;
			const res = await fetch(`/api/content/song?id=${encodeURIComponent(songId)}`);
			if (!res.ok) return null;
			return res.json();
		},
		video: async (videoId: string) => {
			if (!videoId) return null;
			const res = await fetch(`/api/content/video?id=${encodeURIComponent(videoId)}`);
			if (!res.ok) return null;
			return res.json();
		}
	}
